...
void KinectManager::addListeners() {
	ofAddListener(ofEvents().update, this, &KinectManager::_update);
    ofAddListener(ofEvents().exit, this, &KinectManager::_exit);

    bAddedListeners = true;
}

void KinectManager::addDebugDrawListener() {
    ofAddListener(ofEvents().draw, this, &KinectManager::_debugDraw);
    bAddedDrawListener = true;
}

void KinectManager::addKeyListener(){
    ofAddListener(ofEvents().keyPressed, this, &KinectManager::keyPressed);
    bAddedKeyListener=true;
}


void KinectManager :: removeListeners()
{
    if( !bAddedListeners )
        return;
	ofRemoveListener( ofEvents().update, this, &KinectManager :: _update	);
	if(bAddedDrawListener)ofRemoveListener( ofEvents().draw,   this, &KinectManager :: _debugDraw);
	ofRemoveListener( ofEvents().exit,   this, &KinectManager :: _exit	);
    ofRemoveListener(ofEvents().keyPressed, this, &KinectManager::keyPressed);
    
    bAddedListeners = false;
    bAddedDrawListener = false;
    bAddedKeyListener=false;

}

void KinectManager :: removeDebugDrawListener()
{
    if( !bAddedDrawListener)
        return;
	ofRemoveListener( ofEvents().draw,   this, &KinectManager :: _debugDraw	);
    bAddedDrawListener = false;
}

void KinectManager::removeKeyListener(){
    ofRemoveListener(ofEvents().keyPressed, this, &KinectManager::keyPressed);
    bAddedKeyListener=false;
}


void KinectManager::setDraw(bool b) {
	if(doDraw != b) {
		doDraw = b;
		if(doDraw) addDebugDrawListener();
		else removeDebugDrawListener();
	}
}

void KinectManager::showDebug() {
	setDraw(true);
}

void KinectManager::hideDebug() {
	setDraw(false);
}

void KinectManager::toggleDebug() {
	setDraw(!doDraw);
}

bool KinectManager::isDebugDrawing() {
	return doDraw;
}

void KinectManager:: turnOnKeyListener(){
    if(!isKeyListening()){
        addKeyListener();
    }
}

void KinectManager::turnOffKeyListener(){
    if(isKeyListening()){
        removeKeyListener();
    }
}

bool KinectManager::isKeyListening() {
...
void KinectManager :: _update( ofEventArgs &e )
{
    // cout<<"update"<< dimensions.x<<" "<<dimensions.y<<" "<<dimensions.width<<" "<<dimensions.height<<endl;
	update();
    
}

void KinectManager :: _debugDraw( ofEventArgs &e )
{
	debugDraw();
}


void KinectManager :: _exit( ofEventArgs &e )
{
	exit();
}
...
void KinectManager::init(){
   ...
   // ofAddListener(gestGen.gestureRecognized,this,&KinectManager::gestureRecognized);
...
void KinectManager::gestureRecognized(gesture &g){
    cout<<"------------------------FOUND---------------------"<<endl;
    ...
    /*if(g.gesture_name=="Click"){
        ...
        //  feedback();
        clicked=!clicked;
        if(clicked && recordHandTracker.getNumTrackedHands()>0){
...
void KinectManager::setupRecording(string _filename) {
 ...
 */
    ...
    gestGen.setup(&recordContext);
    
    
}

void KinectManager::setupPlayback(string _filename) {
...
void KinectManager::update(){
    //cout<<"updating KinectManager"<<endl;
...
void KinectManager::debugDraw(){
    ...
    //cout<<"debugDraw KinectManager"<<endl;
...
//	statusHardware = statusHardwareStream.str();
    ...
    string keyStatus		= (string)(isKeyListening() ? "Kinect Key On" : "Kinect Key Off");
...
void KinectManager::drawPointCloud(ofxUserGenerator * user_generator, int userID) {
...
string KinectManager::generateFileName() {
...
//--------------------------------------------------------------
void KinectManager::keyPressed(ofKeyEventArgs &e){
    int key=e.key;
...
void KinectManager::exit(){
    ...
    ofRemoveListener(gestGen.gestureRecognized,this,&KinectManager::gestureRecognized);
    }
